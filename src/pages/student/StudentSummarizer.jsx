  ...
  const handleSummarize = async (type) => {
    ...
    try {
      ...
      if (type === 'course') {
        ...
        data = await api.post('/documents/summarize-content', { courseId });
      } else if (type === 'video') {
        if (!courseId || !videoIndex) {
          toast.error('Please enter course ID and video index');
          setLoading(false);
          return;
        }
        data = await api.post('/documents/summarize-content', { courseId, videoIndex });
      }
